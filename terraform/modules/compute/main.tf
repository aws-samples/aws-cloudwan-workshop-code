...
resource "aws_instance" "ec2_instance" {
  ...
  subnet_id                   = values({ for k, v in var.vpc_information.private_subnet_attributes_by_az : split("/", k)[1] => v.id if split("/", k)[0] == "workload" })[count.index]
...
resource "aws_security_group" "eic_sg" {
  name        = "${var.vpc_name}-eic-security-group-${var.project_name}"
...
resource "aws_ec2_instance_connect_endpoint" "eic_endpoint" {
  subnet_id          = values({ for k, v in var.vpc_information.private_subnet_attributes_by_az : split("/", k)[1] => v.id if split("/", k)[0] == "vpc_endpoints" })[0]
